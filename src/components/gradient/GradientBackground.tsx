...
export const GradientBackground: React.FC<GradientBackgroundProps> = ({ groups, settings, pageVh, showOutlines }) => {
  return (
    <div className="absolute inset-0 z-[1] pointer-events-none overflow-hidden" style={{ minHeight: `${pageVh}vh` }}>
      <AnimationStyles />
      {groups.filter(g => g.enabled !== false).map((group) => group.halos.filter(h => h.enabled !== false).map((halo) => (
...
const HaloLayer = React.memo(function HaloLayer({ group, halo, settings, showOutlines }: any) {
  ...
  return (
    <div
      ...
      style={{
        ...
        width: `${size * 2.2}vh`,
        height: `${size * 2.2}vh`,
        transform: `translate(-50%, -50%) scale(${live.stretch / 100}, 1)`,
        opacity: live.opacity * live.intensity * group.intensity * settings.groupIntensity,
        filter: 'none', // Removed expensive CSS blur for massive performance gain
        mixBlendMode: 'normal', // Changed from "screen" to "normal" to save GPU compositing
